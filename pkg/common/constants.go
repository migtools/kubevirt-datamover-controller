...
const (
	...
	// AnnotationSkipQuiesce, when set on a DataUpload, explicitly overrides
	// the controller's automatic guest-agent-based quiesce detection (see
	// IsGuestAgentConnected). Set to "true" to force a crash-consistent
	// backup (skip quiesce, matching VirtualMachineBackup.Spec.SkipQuiesce)
	// even when the guest agent is detected as connected; set to "false" to
	// force a quiesced, application-consistent backup even when it isn't.
	// Any other value, or the annotation being unset, falls back to
	// automatic detection: the controller quiesces only when the VM's
