...
func TestUpdateVMIndex_UnexpectedFullMarksSuperseded(t *testing.T) {
	scheme := getTestScheme()
	...
	tests := []struct {
		...
		existingIndex      *VMIndex
		wantSuperseded     map[string]bool // checkpoint ID → expected Superseded value
	}{
		{
			name:               "unexpected full marks existing chain as superseded",
			backupType:         "full",
			...
			wantSuperseded: map[string]bool{
				"cp-001": true,
				"cp-002": true,
				"cp-003": false, // new full backup
			},
		},
		{
			name:               "normal full backup does not mark superseded",
			backupType:         "full",
			expectedBackupType: "full",
			existingIndex: &VMIndex{
				VMName:    "test-vm",
				Namespace: "test-ns",
				Checkpoints: []CheckpointEntry{
		...
		{
			name:               "empty expected type does not mark superseded",
			backupType:         "full",
			expectedBackupType: "",
			existingIndex: &VMIndex{
				VMName:    "test-vm",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			config := &UploaderConfig{
				...
				ExpectedBackupType: tt.expectedBackupType,
				VMBName:            "vmb-3",
				VeleroBackupName:   "backup-003",
			}
			files := []CheckpointFile{
				{Filename: "vmb-3-disk1.qcow2", DiskName: "disk1", Size: 1024,
					ObjectPath: "checkpoints/test-ns/test-vm/cp-003/vmb-3-disk1.qcow2"},
			}
			...
			for cpID, wantSup := range tt.wantSuperseded {
				cp := findCheckpoint(idx.Checkpoints, cpID)
				if cp == nil {
					t.Errorf("checkpoint %s not found in index", cpID)
					continue
				}
				if cp.Superseded != wantSup {
					t.Errorf("checkpoint %s: Superseded = %v, want %v", cpID, cp.Superseded, wantSup)
				}
			}
		})
	}
